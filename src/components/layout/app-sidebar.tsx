...
export function AppSidebar({ user }: { user: SessionUser }) {
  ...
  useEffect(() => {
    window.addEventListener("qa-sidebar-toggle", toggleSidebar);
    return () => window.removeEventListener("qa-sidebar-toggle", toggleSidebar);
  }, [toggleSidebar]);
  ...
  return (
    <>
      <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
        <SheetContent
          side="left"
          className="w-72 gap-0 border-slate-200 bg-slate-50 p-0 text-slate-950 sm:max-w-72"
        >
          <SheetHeader className="sr-only">
            <SheetTitle>Navigation menu</SheetTitle>
            <SheetDescription>QA Management navigation and account menu.</SheetDescription>
          </SheetHeader>
          <SidebarNavigation
            user={user}
            pathname={pathname}
      ...
      <aside
        className={cn(
          "hidden border-r border-slate-200 bg-slate-50/60 text-slate-950 transition-[width] duration-200 lg:block",
          collapsed ? "w-14" : "w-64",
        )}
      >
        <div className="sticky top-0 h-screen">
          <SidebarNavigation user={user} pathname={pathname} collapsed={collapsed} />
        </div>
      </aside>
    </>
  );
    ...
    <div className="flex h-full flex-col p-2">
      ...
      <div className="mt-6 space-y-1">
        ...
        {secondaryItems.map((item) => (
          <Link
            ...
            title={collapsed ? item.label : undefined}
            className={cn(
              "flex h-9 items-center gap-2 rounded-md px-2 text-sm text-slate-700 transition-colors hover:bg-slate-100",
              collapsed && "justify-center",
            )}
          >
      ...
      <div className="mt-auto space-y-1">
        <details className="group relative">
          <summary
            className={cn(
              "flex cursor-pointer list-none items-center gap-2 rounded-md px-2 py-2 hover:bg-slate-100 [&::-webkit-details-marker]:hidden",
              collapsed && "justify-center",
            )}
          >
          ...
          <div className="absolute bottom-12 left-2 z-20 w-56 rounded-xl border border-slate-200 bg-white p-1.5 shadow-lg">
            <Link href="/profile" onClick={onNavigate} className="block rounded-lg px-3 py-2 text-sm hover:bg-slate-100">Profile</Link>
